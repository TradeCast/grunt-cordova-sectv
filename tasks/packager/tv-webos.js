...
function getValidWebosConfData(configPath) {
  ...
  if (
    typeof webosData.largeicon !== 'string' ||
    webosData.largeicon.length <= 0
  ) {
    ...
    ask = [
      {
        ...
        validate: function (input) {
          return /\d.\d.\d/.test(input)
            ? true
            : 'invalid version string for webos platform';
        }
      }
    ];
    inquirer.prompt(ask).then(function (answers) {
      ...
      },
      {
        ...
        validate: function (input) {
          return /^[a-zA-Z0-9]+$/.test(input)
            ? true
            : 'invalid name for webos platform';
        }
      },
      {
        type: 'input',
  ...
  prepare: function (successCallback, errorCallback, platformName, data) {
    ...
    function buildProject() {
      ...
      if (
        ...
        console.log(
          '   It is not supported on this platform. It would be caused abnormal operation.'
        );
  ...
  build: function (successCallback, errorCallback, data) {
    ...
    child.exec('ares --version', function (err, stdout, stderr) {
      if (err) {
        console.log(stderr);
        throw Error(
          'The command "ares" failed. Make sure you have the latest Webos SDK installed, and the "ares" command (inside the CLI folder) is added to your path.'
        );
      }
      console.log(stdout);
