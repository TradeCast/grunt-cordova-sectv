...
function getNextSemVersion(curver) {
  // just increase revision
...
function getValidTizenConfData(configPath) {
  ...
  if (
    typeof tizenData.packageid !== 'string' ||
    tizenData.packageid.length !== 10
  ) {
    // packageid is invalid
    return null;
  }
  if (
    typeof tizenData.iconpath !== 'string' ||
    tizenData.iconpath.length <= 0
  ) {
    // iconpath is invalid
    return null;
  }
    ...
    ask = [
      {
        ...
        validate: function (input) {
          return /\d.\d.\d/.test(input)
            ? true
            : 'invalid version string for tizen platform';
        }
      }
    ];
    inquirer.prompt(ask).then(function (answers) {
    ...
    ask = [
      {
        ...
        validate: function (input) {
          return /^[a-zA-Z][^~!\.\;\\\/\|\"\'@\#$%<>^&*\()\-=+_\’\n\t\s]*$/.test(
            ...
            ? true
            : 'invalid name for tizen platform';
        }
      },
      {
        type: 'input',
        ...
        validate: function (input) {
          return /[0-9a-zA-Z]{10}/.test(input)
            ? true
            : 'invalid id string for tizen platform';
        }
      },
      {
        type: 'input',
  ...
  if (platformsData) {
    for (i = 0; i < platformsData.length; i++) {
      if (platformsData[i].$.name === 'sectv-tizen') {
        ...
        manualTizenConfData = utils.trim(
          js2xmlparser
            .parse('platform', platformsData[i], {
  ...
  prepare: function (successCallback, errorCallback, platformName, data) {
    ...
    function buildProject() {
      ...
      if (
        ...
        console.log(
          'It would be caused abnormal operation. It is recommended to add csp setting in config.xml.'
        );
    ...
    child.exec('tizen version', function (err, stdout, stderr) {
      ...
      if (result.code) {
        ...
        if (packagePath && packagePath[1]) {
          ...
          console.log(
            'Package created at ' +
              path.join(dest, path.basename(packagePath[1]))
          );
        } else {
